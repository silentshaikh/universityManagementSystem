...
class Student extends Person{
    ...
    constructor(name:string,public email:string,public studentId:number,public subject:string) {
        super(name);
    };
   static enrollStudent(name:string,email:string,id:number,role:string,subject:string,teacher?:string){
        this.studList.push({
            name:name,
            email:email,
            studentId:id,
...
class Department extends Person{
   ...
   static addStudent(name:string,email:string,id:number,subject:string,role:string){
...
while(isCond){
    let universityOptin = await inquirer.prompt([
        ...
        if(universityOptin.UniversityOption === "Enroll Student"){
            while(isCond){
                ...
                if(notDuplicateId?.studentId){
                    ...
                    if(notDuplicateEmail?.email){
                            ...
                            if(userInput.studentName === "" || (userInput.studentName as string).length <3){
                                console.log("Please Enter Your Name and fill the form again and the must be 3 character");
                            } 
                            else{
                                if(!(userInput.studentEmail as string).match(/^([a-z_0-9]+)@([a-z]+)\.([a-z]){2,7}$/)){
                        ...
                        if(userInput.studentName !== "" && (userInput.studentName as string).length >=3&&(userInput.studentEmail as string).match(/^([a-z_0-9]+)@([a-z]+)\.([a-z]){2,7}$/)){
            ...
            let filtStudId:StudentDetail|undefined = studList.find((e) => e.studentId === studId.id);
           ...
           if(filtStudId){
             let slectTeacher = await inquirer.prompt([
                ...
                type:"list", choices:["Tom","Ben","Scarlett","Evan","Tony"],
            },
            ]);
           ...
           isCond = addMore.addMoreTeacher;
            ...
            while(isCond){
                ...
                if(deparmntOption.departmentOption === "Student List"){
                    ...
                    let findAdmin = Teacher.teacherList.find((e) => e.id === checkIdAdmin.adminFind);
                ...
                let duplicateTeacherEmail = studList.find((e) => {
                    return e.email === addTeacher.techrEmail;
                        ...
                        if(duplicateTeacherID?.id){
                            ...
                            if(duplicateTeacherEmail){
                                ...
                                if(addTeacher.techrName === "" || (addTeacher.techrName as string).length<3){
                                    console.log(`Please Enter The  Name of Teacher and fill the form again and the must be 3 character`)
                                }else{
                                    if(!(addTeacher.techrEmail as string).match(/^([a-z_0-9]+)@([a-z]+)\.([a-z]){2,7}$/)){
                            ...
                            if(addTeacher.techrName !== "" && (addTeacher.techrName as string).length >=3&&(addTeacher.techrEmail as string).match(/^([a-z_0-9]+)@([a-z]+)\.([a-z]){2,7}$/)){
                                console.log(`Dear ${addTeacher.techrName}, Your ID is ${newTeacherID}`);
                            }
                        }
                    }else{
                        console.log(`this ID ${checkIdAdmin.adminFind} is not an Admin ID.`)
                    };
